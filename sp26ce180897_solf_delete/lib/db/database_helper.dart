// ============================================================
// 💾 HƯỚNG DẪN THAY ĐỔI DATABASE - SOFT DELETE
// ============================================================
...
//
// ⚠️ LƯU Ý: isDeleted và deletedAt là trường SOFT DELETE - KHÔNG xóa
// ============================================================
...
class DatabaseHelper {
  static const _databaseName = 'art_gallery_soft_delete.db'; // Đổi tên DB để phân biệt
  ...
  Future<void> _onCreate(Database db, int version) async {
    ...
    // Ví dụ: price REAL
    // Ví dụ: imageUrl TEXT
    // ⚠️ LƯU Ý: isDeleted và deletedAt là trường SOFT DELETE - MẶC ĐỊNH CÓ
    await db.execute('''
      CREATE TABLE $artworkTable (
        ...
        -- 🔐 SOFT DELETE FIELDS - Mặc định có sẵn
  ...
  // Lấy danh sách artwork CHƯA XÓA của user
  ...
  // TODO [ADD]: Khi thêm trường mới vào Artwork, thường KHÔNG cần sửa ở đây
  // Vì toJson() đã tự động serialize tất cả fields
  // Chỉ sửa nếu có logic đặc biệt

  /// 🔐 XÓA MỀM: Đánh dấu isDeleted = 1 và lưu thời gian xóa
  /// [REASON] Để lưu lại artwork trong DB, cho phép khôi phục sau
  /// [IMPACT] Nếu không dùng soft delete mà dùng hard delete, sẽ mất dữ liệu vĩnh viễn
  Future<int> softDeleteArtwork(int id) async {
    final db = await database;
    return await db.update(
      artworkTable,
      {
  ...
  /// 🔐 LẤY DANH SÁCH ARTWORK ĐÃ XÓA MỀM của user
  /// [REASON] Để hiển thị trong màn hình Trash
  /// [IMPACT] Nếu không có method này, không thể lấy danh sách đã xóa
  Future<List<Artwork>> getDeletedArtworksByUser(int userId) async {
    final db = await database;
    final maps = await db.query(
      artworkTable,
      where: 'createdBy = ? AND isDeleted = 1',
      whereArgs: [userId],
  ...
  /// 🔐 KHÔI PHỤC: Đánh dấu isDeleted = 0 và xóa deletedAt
  /// [REASON] Để khôi phục artwork đã xóa mềm
  /// [IMPACT] Nếu không có method này, không thể khôi phục đã xóa
  Future<int> restoreArtwork(int id) async {
    final db = await database;
    return await db.update(
      artworkTable,
      {
  ...
  Future<int> permanentDeleteArtwork(int id) async {
    final db = await database;
  ...
  /// 🔐 XÓA VĨNH VIỄN TẤT CẢ: Xóa vĩnh viễn tất cả artwork đã xóa mềm của user
  /// [REASON] Để empty trash (xóa tất cả đã xóa mềm)
  /// [IMPACT] Hành động này KHÔNG THỂ HOÀN TÁC
  Future<int> emptyTrash(int userId) async {
    final db = await database;
    return await db.delete(
      artworkTable,
      where: 'createdBy = ? AND isDeleted = 1',
  ...
  /// 🔐 XÓA MỀM artwork (sử dụng soft delete thay vì xóa vĩnh viễn)
  /// ⚠️ ĐÂY LÀ ALIAS CỦA softDeleteArtwork - Dùng CHO TƯƠNG THÍCH NGƯỢC
  Future<int> deleteArtwork(int id) async {
    return await softDeleteArtwork(id);
  }
}
