// ============================================================
// 💾 HƯỚNG DẪN THÊM TRƯỜNG MỚI VÀO LIST SCREEN - SOFT DELETE
// ============================================================
// KHI THÊM TRƯỜNG MỚI VÀO ARTWORK:
//
// 1. SUBTITLE - Hiển thị trường mới trong ListTile subtitle:
//    Tìm Text widget subtitle và thêm: + '\nField: ${a.newField}'
//
// 2. SEARCH - Tìm kiếm theo trường mới:
//    Trong _onSearch(), thêm điều kiện:
//    || a.newField.toLowerCase().contains(query)
//
// 3. CONTROLLER - Dispose controller mới (nếu có):
//    Thêm: _newController.dispose();
//
// ⚠️ LƯU Ý: KHÔNG thay đổi logic soft delete (đã xử lý sẵn)
// ============================================================
...
class _ListScreenState extends ConsumerState<ListScreen> {
      ...
      if (query.isEmpty) {
        ...
        _filtered = _artworks
            .where(
              ...
              // Ví dụ: || a.price?.toString().contains(query) == true
            )
            .toList();
      }
    });
  ...
  // 🔐 SOFT DELETE: Đổi từ hard delete sang soft delete
  Future<void> _deleteArtwork(Artwork artwork) async {
    ...
    if (ok == true && artwork.id != null) {
      // 🔐 Gọi softDelete thay vì delete (hard delete vĩnh viễn)
      await DatabaseHelper.instance.softDeleteArtwork(artwork.id!);
      if (mounted) {
        setState(() {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Artwork moved to trash')),
        );
      }
    }
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        actions: [
          // 🔐 Nút Trash - mở màn hình thùng rác
          IconButton(
            ...
            onPressed: () async {
              ...
              _load(); // Refresh sau khi quay lại từ trash
            },
          ),
